...
/****** Object:  View [dbo].[sf_Account]    Script Date: 08/27/2015 14:50:35 ******/
SET ANSI_NULLS ON
GO
...
/*
	CASE
		WHEN orderCustomerCategory IN ('4', '5', '6', '8') AND
			 LTRIM(RTRIM(customerAddress1)) <> '' AND
			 LTRIM(RTRIM(customerAddress2)) <> '' AND
			 LTRIM(RTRIM(customerAddress3)) <> ''
			THEN LTRIM(RTRIM(customerAddress1)) + ', ' +
				 LTRIM(RTRIM(customerAddress2)) + ', ' +
				 LTRIM(RTRIM(customerAddress3))
		WHEN orderCustomerCategory IN ('4', '5', '6', '8') AND
			 LTRIM(RTRIM(customerAddress1)) <> '' AND
			 LTRIM(RTRIM(customerAddress2)) <> ''
...
-- Record Date
/*
	(SELECT TOP 1 SO.orderCreationDate
	 FROM tb_Customer S LEFT OUTER JOIN tb_salesOrder SO
		ON S.customerSalesOrderId = SO.salesOrderId
	 WHERE S.customerIdentificationNo = C.customerIdentificationNo
	 ORDER BY SO.orderCreationDate ASC)
							[enterDate],
	(SELECT TOP 1 SO.orderCreationDate
	 FROM tb_Customer S LEFT OUTER JOIN tb_salesOrder SO
		ON S.customerSalesOrderId = SO.salesOrderId
	 WHERE S.customerIdentificationNo = C.customerIdentificationNo
	 ORDER BY SO.orderCreationDate DESC)
							[lastUpdate]
*/
	C.enterDate,
	C.lastUpdate
